...
void main() {
  ...
  Future<void> preencherCamposValidos(WidgetTester tester, {String email = 'novo@teste.com'}) async {
    await tester.enterText(find.widgetWithText(TextFormField, 'Nome'), 'Pescador');
  ...
  testWidgets('mostra erro de email invalido', (tester) async {
    ...
    await tester.enterText(find.widgetWithText(TextFormField, 'Email'), 'email-invalido');
    await tester.tap(find.widgetWithText(ElevatedButton, 'Criar conta'));
    await tester.pump();
  ...
  testWidgets('mostra erro inline quando confirmar senha difere', (tester) async {
    await tester.pumpWidget(buildApp());

    await tester.enterText(find.widgetWithText(TextFormField, 'Nome'), 'Pescador');
    await tester.enterText(find.widgetWithText(TextFormField, 'Email'), 'novo@teste.com');
    await tester.enterText(find.widgetWithText(TextFormField, 'Senha'), '123456');
    await tester.enterText(find.widgetWithText(TextFormField, 'Confirmar senha'), 'outraSenha');
    await tester.tap(find.widgetWithText(ElevatedButton, 'Criar conta'));
    await tester.pump();
